...
function Cart() {
    ...
    const {foodList, increaseQuantity, decreaseQuantity, quantity, removeFromCart } = useContext(StoreContext);

    const cartItem = foodList.filter(food => (quantity[food.id] || 0) > 0);
                                        ...
                                        <div key={food.id} className="row cart-item mb-3">
                                            ...
                                            <div className="col-md-2">
                                                <div className="input-group">
                                                    ...
                                                    <input style={{ maxWidth: '100px' }} type="text" className="form-control  form-control-sm text-center quantity-input" value={quantity[food.id]} readOnly />
                                                    <button className="btn btn-outline-secondary btn-sm" type="button" onClick={() => increaseQuantity(food.id)}>+</button>
                                                </div>
                                            </div>
                                            <div className="col-md-2 text-end">
                                                <p className="fw-bold">₹{(food.price * quantity[food.id]).toFixed(2)}</p>
            ...
            <div className="col-lg-4">
                <div className="card cart-summary">
                    <div className="card-body">
                        ...
                        <button className="btn btn-primary w-100" disabled={cartItem.length === 0} onClick={() => navigate('/placeorder')}>
                            Proceed to Checkout
                        </button>
                    </div>
                </div>
